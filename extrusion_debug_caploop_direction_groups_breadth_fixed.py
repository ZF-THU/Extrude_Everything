...
def validate_side_clusters_by_cap_candidates(
    ...
    Requested order:
      ...
      2. After all full trials are done, only families that still failed evaluate
         all delete-one children.
      3. After all delete-one trials are done, only families that still failed
         evaluate all unique delete-two children.
      4. For each family, once a cap is found at a level, that family stops and
         no smaller subsets are evaluated for that family.

    Delete-two duplicates are avoided by caching final side-index sets, so
    deleting A then B and deleting B then A is evaluated once.
    """
    if model is None:
        return None, []
    ...
    # Initialize per-family state/debug first.  No cap is computed here.
    states = []
    for rank, entry in enumerate(cluster_debug):
        ...
        details["search_strategy"] = "direction_groups_breadth_first_full_then_delete_one_then_delete_two"
    ...
    # ------------------------------------------------------------------
    # Level 1: for all still-failed families, evaluate every delete-one child.
    # ------------------------------------------------------------------
    for state in states:
        if state["done"]:
            continue
        parent_strokes = state["parent_strokes"]
        if len(parent_strokes) < 3:
            state["details"]["delete_one_trials"] = []
            continue

        trials = []
        for i, s_removed in enumerate(parent_strokes):
            child = [s for j, s in enumerate(parent_strokes) if j != i]
            trial = eval_side_for_state(state, child, [int(s_removed["index"])])
            if trial is not None:
                trials.append(trial)
        state["details"]["delete_one_trials"] = [_trial_summary_for_debug(t) for t in trials]

        best = _choose_best_successful_trial(trials)
        if best is not None:
            mark_family_success(state, best, "delete_one")

    # ------------------------------------------------------------------
    # Level 2: for all still-failed families, evaluate every unique delete-two child.
    # ------------------------------------------------------------------
    import itertools
    for state in states:
        if state["done"]:
            continue
        parent_strokes = state["parent_strokes"]
        if len(parent_strokes) < 4:
            state["details"]["delete_two_trials"] = []
            state["details"]["invalid_no_cap"] = True
            continue

        trials = []
        for i, j in itertools.combinations(range(len(parent_strokes)), 2):
            removed = [int(parent_strokes[i]["index"]), int(parent_strokes[j]["index"])]
            child = [s for k, s in enumerate(parent_strokes) if k not in (i, j)]
            trial = eval_side_for_state(state, child, removed)
            if trial is not None:
                trials.append(trial)
        state["details"]["delete_two_trials"] = [_trial_summary_for_debug(t) for t in trials]

        best = _choose_best_successful_trial(trials)
        if best is not None:
            mark_family_success(state, best, "delete_two")
        else:
            state["details"]["invalid_no_cap"] = True

    # Collect successes.  Final choice keeps direction-family rank priority.
    ...
    if not successful_states:
        ...
        model["cap_validation_message"] = "No direction family produced a legal cap after breadth-first full/delete-one/delete-two search."
        return model, []

    selected_state = min(successful_states, key=lambda st: st["rank"])
    selected_entry_base = selected_state["entry"]
    selected_trial = selected_state["best_trial"]
    selected_cap = selected_trial["best_cap"]
    selected_side_set = set(selected_trial.get("side_indices", []))

    synthetic_entry = dict(selected_entry_base)
    synthetic_entry["strokes"] = [s for s in selected_state["parent_strokes"] if int(s["index"]) in selected_side_set]
    synthetic_entry["indices"] = selected_trial.get("side_indices", [])
    synthetic_entry["direction"] = selected_trial.get("direction", selected_entry_base.get("direction"))
    synthetic_entry["score"] = float(selected_trial.get("score", selected_entry_base.get("score", 0.0)))
    synthetic_entry["details"] = selected_state["details"]
    ...
    validated_model["cap_validation_message"] = (
        "Direction groups are evaluated breadth-first: all full clusters first, then "
        "delete-one for failed families, then unique delete-two for still-failed families. "
        "Each family stops at its first successful level; final selection keeps family rank priority."
    )
    validated_model["successful_cap_clusters"] = model.get("successful_cap_clusters", [])
    return validated_model, [selected_cap]
