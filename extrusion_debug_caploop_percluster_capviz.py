...
def validate_side_clusters_by_cap_candidates(
    ...
    """
    Compute cap candidates for every non-rejected side cluster with at least 2 strokes,
    then select the first ranked valid side cluster.

    Procedure:
      1. Use model['cluster_debug'], already ranked from high score to low score.
      2. Skip clusters that were already hard-rejected by side scoring
         (for example invalid_connected_cluster=True or score=-1e18).
      3. For every remaining cluster whose stroke count >= 2, remove that cluster's
         strokes from all infos and compute stroke-loop cap candidates from the remainder.
      4. For each checked cluster, keep ONLY the largest-area cap candidate in debug
         output. Other cap candidates are counted but not printed as candidates.
      5. Choose the first ranked selectable cluster that has a legal cap.

    Hard-rejected clusters do NOT compute cap candidates.
    """
    if model is None:
        return None, []
    ...
    # Non-cluster models, e.g. manual VP, keep old behavior but return only the
    # largest-area cap candidate.
    ...
    for rank, entry in enumerate(cluster_debug):
        ...
        # Only the largest-area cap candidate is exposed for this side cluster.
        ...
        # User-requested rule: hard-rejected clusters DO NOT compute cap.
        # Examples: connected_pairs hard reject, or score already forced to -1e18.
        pre_rejected = (
        ...
        # User-requested rule: only clusters with >=2 side strokes compute cap.
        if n_strokes < 2:
            details["cap_validation_skipped"] = True
        ...
        # Only the largest-area candidate is stored/output for this cluster.
    ...
    validated_model["cap_validation_message"] = (
        "Computed best cap for every non-rejected side cluster with n>=2; selected the first "
        "ranked selectable side cluster that produced a legal cap."
    )
...
def save_per_cluster_side_cap_outputs(debug_dir, img_shape, model, infos, args):
    """Save side/cap visualizations for every ranked side cluster.

    For each cluster:
      - always save side strokes;
      - if not hard-rejected and n>=2, save the largest-area cap candidate;
      - save a combined side+best-cap overlay.
    """
    if debug_dir is None or model is None:
        return
    cluster_debug = model.get("cluster_debug", [])
    ...
    with open(summary_path, "w", encoding="utf-8") as f:
        ...
        f.write("For each ranked side cluster, side image is always saved.\n")
        f.write("Best cap is computed only when cluster is not pre-rejected and n>=2.\n")
        f.write("Each cluster stores only the largest-area cap candidate.\n\n")
        ...
        for rank, entry in enumerate(cluster_debug):
            ...
            if pre_rejected:
                ...
                # Recompute to get the actual mask for visualization. The text/debug
                # fields were already computed earlier; this is only for images.
                candidates = extract_cap_loop_candidates_from_strokes(
                    img_shape,
                    infos,
                    entry.get("strokes", []),
