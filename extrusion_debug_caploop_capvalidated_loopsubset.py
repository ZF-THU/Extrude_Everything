...
def validate_side_clusters_by_cap_candidates(
    ...
    """
    Select side cluster using cap feasibility after side scoring.

    Procedure:
      1. Use model['cluster_debug'], already ranked from high score to low score.
      2. For each non-rejected side cluster, remove its strokes from all infos.
      3. Compute stroke-loop cap candidates from the remaining strokes.
      4. If no legal closed-loop cap candidate exists, reject that side cluster.
      5. Choose the first ranked side cluster that yields at least one legal cap.

    This implements:
      high-score side cluster -> cap candidate check -> reject until first cap appears.
    """
    if model is None:
        return None, []
    ...
    # Non-cluster models, e.g. manual VP, keep old behavior.
    if not cluster_debug:
        ...
        model["cap_validated"] = len(candidates) > 0
    ...
    for rank, entry in enumerate(cluster_debug):
        ...
        # Keep already hard-rejected clusters visible, but do not try to select them.
        if details.get("invalid_connected_cluster", False) or float(entry.get("score", -1e18)) <= -1e17:
        ...
        details["cap_candidate_strokes"] = [c.get("stroke_indices", []) for c in trial_candidates]
        details["cap_candidate_scores"] = [float(c.get("score", 0.0)) for c in trial_candidates]
    ...
    validated_model["cap_validation_message"] = "Selected first ranked side cluster that produced a legal cap candidate."
    ...
    with open(path, "w", encoding="utf-8") as f:
        ...
        for entry in cluster_debug:
            ...
            if details.get("cap_validation_checked", False):
                f.write(
                    f"    cap_validation: candidates={details.get('cap_candidate_count', 0)}, "
                    f"candidate_strokes={details.get('cap_candidate_strokes', [])}, "
                    f"candidate_scores={details.get('cap_candidate_scores', [])}, "
                    f"invalid_no_cap={details.get('invalid_no_cap', False)}, "
...
def main():
    ...
    # try side clusters from high score to low score; reject a side cluster if
    # removing it leaves no legal closed-loop cap candidate.
