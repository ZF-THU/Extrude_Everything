...
def build_direction_clusters(
    ...
    """
    Build coarse direction groups first, then create one maximal side cluster per group.

    This follows the requested order:
      1. compute PCA axis angle / direction for every line stroke;
      2. build direction groups in direction space;
      3. for each direction group, generate exactly one maximal cluster first.

    Implementation detail:
      - Build an undirected graph over line_strokes.
      - Add an edge if two strokes have UNORIENTED PCA direction angle <= angle_thresh.
      - Direction groups are connected components of this graph.
      - Each connected component is the initial maximal cluster for that direction family.

    No all-subset / all-clique enumeration happens here.  Local delete-one/delete-two
    search is done later during cap validation, level by level across all families.
    """
    n = len(line_strokes)
    if n == 0:
        return []

    # Build direction-space graph.
    adj = {i: set() for i in range(n)}
    pair_angles = {}
    for i in range(n):
        for j in range(i + 1, n):
            a = angle_between_dirs(line_strokes[i]["direction"], line_strokes[j]["direction"])
            pair_angles[(i, j)] = float(a)
            if a <= angle_thresh:
                adj[i].add(j)
                adj[j].add(i)

    # Connected components in direction space.
    visited = set()
    groups = []
    for i in range(n):
        if i in visited:
    ...
    for gid, comp in enumerate(groups):
        strokes = [line_strokes[i] for i in comp]
        indices = [int(s["index"]) for s in strokes]
        # For debug: the maximum pair angle inside the connected component can be
        # larger than angle_thresh because connected components are transitive.
        max_pair_angle = 0.0
        mean_pair_angle = 0.0
        vals = []
        for a in range(len(comp)):
            for b in range(a + 1, len(comp)):
                ii, jj = sorted((comp[a], comp[b]))
                vals.append(pair_angles.get((ii, jj), 0.0))
        if vals:
            max_pair_angle = float(max(vals))
            mean_pair_angle = float(sum(vals) / len(vals))

        out.append({
            "strokes": strokes,
            "source": "direction_group_cc",
            "direction_group_id": int(gid),
            "direction_group_indices": indices,
            "direction_group_max_pair_angle": max_pair_angle,
            "direction_group_mean_pair_angle": mean_pair_angle,
        })

    # Direction groups with more support first.
    out.sort(
        key=lambda e: (
            -len(e["strokes"]),
            -sum(float(st["arc"]) for st in e["strokes"]),
            tuple(sorted(int(st["index"]) for st in e["strokes"])),
        )
    )

    # Reassign group ids after ranking for easier debug reading.
    for new_gid, e in enumerate(out):
        e["direction_group_rank"] = int(new_gid)
...
def fallback_parallel_direction(
    ...
    for ci, cluster_entry in enumerate(cluster_entries):
        ...
        # Direction-family priority is used for validation order.  It does not
        # hard-reject connected families, because connected maximal families can
        # still become valid after delete-one/delete-two local repair.
        details["family_priority_score"] = float(100000.0 * len(cluster) + details.get("total_len", 0.0))
    ...
    # Rank by maximal direction-family size/length first, not by hard-rejected
    # side score.  A maximal family may be connected and score=-inf, but it is
    # still the correct parent for delete-one/delete-two local search.
    ranked_clusters = sorted(
        scored_clusters,
        key=lambda x: (
            -int(x.get("details", {}).get("n", len(x.get("strokes", [])))),
            -float(x.get("details", {}).get("total_len", 0.0)),
            -float(x.get("details", {}).get("family_priority_score", 0.0)),
        ),
    )
    # In the new breadth-first direction-family search, a maximal direction
    # family may be connected and therefore get score = -1e18 from
    # score_side_cluster(). That is not fatal: cap validation can still repair it
    # by trying delete-one/delete-two children. If all full families are hard-
    # rejected, best_i remains None, so choose the top ranked family as the
    # temporary model parent instead of crashing.
    if best_i is None:
...
def _cap_trial_for_side_strokes(
    side_strokes,
    infos,
    image_shape,
    endpoint_tol=12.0,
    ...
    """Evaluate one candidate side-stroke set and return a debug record."""
    removed_indices = [] if removed_indices is None else [int(x) for x in removed_indices]
    side_strokes = list(side_strokes)
    side_indices = [int(s["index"]) for s in side_strokes]
    direction = mean_direction(side_strokes) if side_strokes else np.array([1.0, 0.0], dtype=np.float64)
    side_score, side_details = score_side_cluster(
        side_strokes,
        direction,
        same_loop_endpoint_tol=same_loop_endpoint_tol,
    ) if len(side_strokes) > 0 else (-1e18, {})
    ...
    if len(side_strokes) < 2:
        trial["skipped"] = True
        trial["skip_reason"] = "n_lt_2"
        return trial

    if side_details.get("invalid_connected_cluster", False):
        trial["skipped"] = True
        trial["skip_reason"] = "invalid_connected"
        return trial
    ...
    trial["cap_count"] = int(len(candidates))
    if best_cap is not None:
        trial["best_cap"] = best_cap
        trial["best_cap_area"] = int(best_cap.get("area", 0))
        trial["best_cap_score"] = float(best_cap.get("score", 0.0))
        trial["best_cap_strokes"] = [int(x) for x in best_cap.get("stroke_indices", [])]
    return trial


def _choose_best_successful_trial(trials):
    successes = [t for t in trials if (not t.get("skipped", False)) and t.get("best_cap") is not None]
    if not successes:
        return None
    return max(successes, key=lambda t: (int(t.get("best_cap_area", 0)), float(t.get("best_cap_score", 0.0)), int(t.get("n", 0))))


def _trial_summary_for_debug(trial):
    return {
        "removed_indices": trial.get("removed_indices", []),
        "side_indices": trial.get("side_indices", []),
        "n": int(trial.get("n", 0)),
        "skipped": bool(trial.get("skipped", False)),
        "skip_reason": trial.get("skip_reason", ""),
        "cap_count": int(trial.get("cap_count", 0)),
        "best_cap_area": int(trial.get("best_cap_area", 0)),
        "best_cap_score": float(trial.get("best_cap_score", 0.0)),
        "best_cap_strokes": trial.get("best_cap_strokes", []),
        "connected_pairs": trial.get("details", {}).get("connected_pairs", []),
    }
...
def validate_side_clusters_by_cap_candidates(
    ...
    """
    Breadth-first cap validation over direction families.

    Updated requested order:
      1. Every direction family first evaluates its full/maximal cluster.
      2. After all full trials are done, only families that still failed evaluate
         all unique delete-one children.
      3. After that, only families that still failed evaluate all unique delete-two
         children.
      4. Continue delete-k breadth-first until either:
           - that family finds a legal cap, or
           - the remaining side cluster would contain fewer than 2 strokes.
      5. For each family, once a cap is found at a level, that family stops and
         no smaller subsets are evaluated for that family.

    Delete-k duplicates are avoided by caching final side-index sets, so different
    deletion orders that lead to the same subset are evaluated only once.
    """
    if model is None:
        return None, []
    ...
    for rank, entry in enumerate(cluster_debug):
        ...
        parent_strokes = list(entry.get("strokes", []))
        parent_indices = [int(s["index"]) for s in parent_strokes]
        ...
        details["search_strategy"] = "direction_groups_breadth_first_full_then_delete_k_until_cap_or_lt2"
        details["selected_search_level"] = ""
        details["selected_removed_indices"] = []
        details["selected_side_indices"] = []
        details["full_trial"] = None
        details["delete_one_trials"] = []
        details["delete_two_trials"] = []
        details["delete_level_trials"] = {}
        details["max_delete_level_attempted"] = 0
        ...
        state = {
            ...
            "evaluated_side_sets": set(),
            "done": False,
            "best_trial": None,
            "selected_level": "",
        }

        if len(parent_strokes) < 2:
    ...
    def eval_side_for_state(state, strokes, removed):
        key = tuple(sorted(int(s["index"]) for s in strokes))
        if key in state["evaluated_side_sets"]:
            return None
        state["evaluated_side_sets"].add(key)
        return _cap_trial_for_side_strokes(
            strokes,
            infos,
            image_shape,
            endpoint_tol=endpoint_tol,
    ...
    def mark_family_success(state, trial, level):
        details = state["details"]
        best_cap = trial["best_cap"]
        ...
        details["cap_candidate_count"] = int(trial.get("cap_count", 0))
        ...
        details["selected_search_level"] = level
        details["selected_removed_indices"] = trial.get("removed_indices", [])
        details["selected_side_indices"] = trial.get("side_indices", [])

        state["done"] = True
        state["best_trial"] = trial
        state["selected_level"] = level

    # Level 0: evaluate FULL/maximal cluster for every direction family.
    for state in states:
        if state["done"] and state["best_trial"] is None:
            continue
        parent_strokes = state["parent_strokes"]
        if len(parent_strokes) < 2:
            continue
        state["details"]["cap_validation_checked"] = True
        trial = eval_side_for_state(state, parent_strokes, [])
        state["details"]["full_trial"] = _trial_summary_for_debug(trial) if trial is not None else None
        best = _choose_best_successful_trial([trial] if trial is not None else [])
        if best is not None:
            mark_family_success(state, best, "full")

    # Levels k>=1: breadth-first over all families. Stop a family when it first succeeds.
    import itertools
    max_parent_n = max((len(st["parent_strokes"]) for st in states), default=0)
    max_delete = max(0, max_parent_n - 2)

    for k in range(1, max_delete + 1):
        any_family_attempted = False
        key_name = f"delete_{k}_trials"
        for state in states:
            if state["done"]:
                continue
            parent_strokes = state["parent_strokes"]
            n_parent = len(parent_strokes)
            remaining = n_parent - k
            if remaining < 2:
                state["details"]["delete_level_trials"][k] = []
                if k == 1:
                    state["details"]["delete_one_trials"] = []
                elif k == 2:
                    state["details"]["delete_two_trials"] = []
                continue

            any_family_attempted = True
            trials = []
            for remove_pos in itertools.combinations(range(n_parent), k):
                remove_pos_set = set(remove_pos)
                child = [s for idx, s in enumerate(parent_strokes) if idx not in remove_pos_set]
                removed = [int(parent_strokes[idx]["index"]) for idx in remove_pos]
                trial = eval_side_for_state(state, child, removed)
                if trial is not None:
                    trials.append(trial)

            summaries = [_trial_summary_for_debug(t) for t in trials]
            state["details"]["delete_level_trials"][k] = summaries
            state["details"]["max_delete_level_attempted"] = max(int(state["details"].get("max_delete_level_attempted", 0)), k)
            if k == 1:
                state["details"]["delete_one_trials"] = summaries
            elif k == 2:
                state["details"]["delete_two_trials"] = summaries
            else:
                state["details"][key_name] = summaries

            best = _choose_best_successful_trial(trials)
            if best is not None:
                mark_family_success(state, best, f"delete_{k}")

        if not any_family_attempted:
            break

    # mark final failures
    for state in states:
        if state.get("best_trial") is None and not state["details"].get("cap_validation_skipped", False):
            state["details"]["invalid_no_cap"] = True

    successful_states = [st for st in states if st.get("best_trial") is not None]
    successful_cap_clusters = []
    for st in successful_states:
        successful_cap_clusters.append({
            "rank": int(st["rank"]),
            "cluster_id": int(st["entry"].get("cluster_id", -1)),
            "side_set": set(st["best_trial"].get("side_indices", [])),
            "search_level": st.get("selected_level", ""),
            "removed_indices": list(st["best_trial"].get("removed_indices", [])),
        })
    ...
    model["successful_cap_clusters"] = [
        {
            ...
            "search_level": x.get("search_level", ""),
            "removed_indices": x.get("removed_indices", []),
        }
    ...
    if not successful_states:
        ...
        model["cap_validation_message"] = "No direction family produced a legal closed-loop cap candidate."
        return model, []

    selected_state = min(successful_states, key=lambda st: st["rank"])
    selected_state["details"]["selected_by_cap_validation"] = True
    ...
    validated_model["selected_cluster_id"] = int(selected_state["entry"].get("cluster_id", -1))
    validated_model["selected_search_level"] = selected_state.get("selected_level", "")
    validated_model["selected_removed_indices"] = list(selected_state["best_trial"].get("removed_indices", []))
    validated_model["cap_validation_failed"] = False
    validated_model["cap_validation_message"] = (
        "Computed best cap for each direction family breadth-first: full first for all families, "
        "then delete-k for all still-failed families, continuing until a family finds a cap or "
        "its side cluster would have fewer than 2 strokes."
    )
    validated_model["successful_cap_clusters"] = model.get("successful_cap_clusters", [])

    selected_side_indices = set(int(x) for x in selected_state["best_trial"].get("side_indices", []))
    validated_model["inliers"] = [s for s in infos if int(s["index"]) in selected_side_indices]
    validated_model["score"] = float(selected_state["entry"].get("score", validated_model.get("score", 0.0)))

    return validated_model, [selected_state["best_trial"]["best_cap"]]



# ============================================================
# 7. Visualization/debug helpers
# ============================================================
        ...
        for entry in cluster_debug:
            ...
            if details.get("cap_validation_checked", False):
                f.write(
                    ...
                    f"search_level={details.get('selected_search_level', '')}, "
                    f"removed={details.get('selected_removed_indices', [])}, "
                    f"selected_side={details.get('selected_side_indices', [])}\n"
                )
                if details.get("full_trial", None) is not None:
                    f.write(f"    full_trial: {details.get('full_trial')}\n")
                if details.get("delete_one_trials", []):
                    f.write("    delete_one_trials:\n")
                    for tr in details.get("delete_one_trials", []):
                        f.write(f"      {tr}\n")
                if details.get("delete_two_trials", []):
                    f.write("    delete_two_trials:\n")
                    for tr in details.get("delete_two_trials", []):
                        f.write(f"      {tr}\n")
...
def main():
    ...
    # Step 1 debug: save immediately after preprocessing/skeletonization.
    if args.debug_dir is not None:
        cv2.imwrite(os.path.join(args.debug_dir, "00_input.png"), img)
        cv2.imwrite(os.path.join(args.debug_dir, "01_binary.png"), bw)
        cv2.imwrite(os.path.join(args.debug_dir, "02_skeleton.png"), skel)
        cv2.imwrite(os.path.join(args.debug_dir, "02b_skeleton_nodes.png"), draw_skeleton_nodes_debug(skel))

    traced_strokes = trace_strokes(skel, min_pixels=args.trace_min_pixels)

    # Step 2 debug: save raw graph-traced strokes before corner splitting.
    if args.debug_dir is not None:
        cv2.imwrite(
            os.path.join(args.debug_dir, "03a0_traced_strokes_before_corner_split.png"),
            draw_strokes_image(img.shape, traced_strokes, thickness=2, annotate=True),
        )
    ...
    # Step 3 debug: save strokes after geometry-based corner splitting.
    # This is the stroke set used by subsequent stages unless merge is enabled.
    if args.debug_dir is not None:
        cv2.imwrite(
            os.path.join(args.debug_dir, "03a_raw_strokes.png"),
            draw_strokes_image(img.shape, raw_strokes, thickness=2, annotate=True),
        )
    ...
    # Step 4 debug: save post-merge strokes immediately. If merge is disabled,
    # this should match 03a_raw_strokes.png.
    if args.debug_dir is not None:
        cv2.imwrite(
            os.path.join(args.debug_dir, "03b_merged_strokes.png"),
            draw_strokes_image(img.shape, merged_strokes, thickness=2, annotate=True),
        )

    infos = build_stroke_infos(merged_strokes)

    # Step 5 debug: save stroke geometry information immediately.
    if args.debug_dir is not None:
        cv2.imwrite(
            os.path.join(args.debug_dir, "04_stroke_info.png"),
            draw_stroke_infos_image(img.shape, infos, thickness=2),
        )
    model, line_strokes = choose_extrusion_model(args, infos, img.shape, skel)

    # Step 6 debug: save line candidates and stroke direction reports immediately,
    # before cap validation. If the later stages fail, these files still exist.
    if args.debug_dir is not None:
        cv2.imwrite(
            os.path.join(args.debug_dir, "05_line_stroke_candidates.png"),
            draw_line_stroke_candidates_image(img.shape, line_strokes, thickness=3),
        )
        write_stroke_direction_debug_report(
            os.path.join(args.debug_dir, "05a_stroke_directions.txt"),
            infos,
            line_strokes,
        )
        cv2.imwrite(
            os.path.join(args.debug_dir, "05a_stroke_directions.png"),
            draw_stroke_directions_image(img.shape, infos, line_strokes),
        )
        if model is not None:
            save_cluster_debug_outputs(args.debug_dir, img.shape, model)
    ...
    # Step 7 debug: save cluster/cap-validation results immediately.
    if args.debug_dir is not None:
        save_cluster_debug_outputs(args.debug_dir, img.shape, model)
        save_per_cluster_side_cap_outputs(args.debug_dir, img.shape, model, infos, args)
    ...
    # Step 8 debug: save final side removal, cap candidate, and report immediately.
    if args.debug_dir is not None:
        cv2.imwrite(
            os.path.join(args.debug_dir, "06_selected_side_strokes.png"),
            draw_selected_side_strokes_image(img.shape, model, thickness=4),
        )
        cv2.imwrite(
            os.path.join(args.debug_dir, "07_non_side_skeleton.png"),
            draw_non_side_skeleton_image(skel, side_mask, non_side),
        )
        cv2.imwrite(
            os.path.join(args.debug_dir, "08_cap_candidates.png"),
            draw_cap_candidates_debug(img.shape, candidates, max_draw=8),
        )
        save_debug_report(
            os.path.join(args.debug_dir, "debug_report.txt"),
            args,
            raw_strokes,
            merged_strokes,
            infos,
