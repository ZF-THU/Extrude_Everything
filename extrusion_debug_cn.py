...
def merge_strokes_by_endpoint(strokes, max_gap=10.0, max_angle=40.0, max_iters=80, min_length_after_merge=3):
                ...
                for end_i in [0, 1]:
                    for end_j in [0, 1]:
                        ok, gap, angle = can_merge_strokes(
                            ...
                            max_gap=max_gap, max_angle=max_angle
                        )
                        if not ok:
                            continue
                        cost = gap + 0.2 * angle
                        if best is None or cost < best["cost"]:
                            best = {"i": i, "j": j, "end_i": end_i, "end_j": end_j, "cost": cost}
...
def fallback_parallel_direction(line_strokes, angle_cluster_thresh=18.0):
    ...
    for s in line_strokes:
        ...
        group = [t for t in line_strokes if angle_between_dirs(base_dir, t["direction"]) <= angle_cluster_thresh]
    ...
    return {"mode": "parallel", "direction": v, "score": float(score), "inliers": best_group}
...
def choose_extrusion_model(args, infos, img_shape, skel):
    ...
    if args.force_parallel:
        return fallback_parallel_direction(line_strokes, angle_cluster_thresh=args.parallel_angle_thresh), line_strokes
    ...
    parallel_model = fallback_parallel_direction(line_strokes, angle_cluster_thresh=args.parallel_angle_thresh)
    model = vp_model
...
def save_debug_report(path, args, raw_strokes, merged_strokes, infos, line_strokes, model, candidates):
    with open(path, "w", encoding="utf-8") as f:
        ...
        for i, c in enumerate(candidates):
            ...
            f.write(f"  candidate {i:03d}: area={c['area']}, endpoints={c['endpoints']}, closedness={c['closedness']:.2f}, score={c['score']:.1f}, center={ctr_txt}\n")
...
def print_debug(raw_strokes, merged_strokes, infos, line_strokes, model, candidates, output, debug_dir):
    ...
    for i, c in enumerate(candidates[:8]):
        ...
        print(f"  #{i+1}: area={c['area']}, endpoints={c['endpoints']}, closedness={c['closedness']:.2f}, score={c['score']:.1f}, center={ctr_text}")
...
def main():
    ...
    if model is None:
        print("[warning] Could not estimate extrusion direction.")
        print("[warning] Saving debug outputs without final side/cap detection.")
        ...
        raise RuntimeError(
            "Could not estimate extrusion direction, but debug images were saved. "
            "Check 03b_merged_strokes.png and 04_stroke_info.png. "
            "Usually this means no stroke passed the straightness/min-length filter."
        )
    ...
    candidates = extract_cap_candidates(non_side, min_pixels=args.min_cap_pixels)
